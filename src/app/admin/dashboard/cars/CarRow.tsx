//src-app-admin-dashboard-cars-CarRow-tsx
...
interface Car {
  id_cars: string; // REVISI: Mengubah id menjadi id_cars
  brand: string;
  model: string;
...
export default function CarRow({ car, onRefresh }: CarRowProps) {
  ...
  const handleUpdate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // REVISI: Melempar car.id_cars
      await updateCar(car.id_cars, formData);
      setNotification({
        type: "success",
        ...
        <div className="bg-white/80 backdrop-blur-xl border border-neutral-200 p-8 rounded-[2rem] max-w-sm w-full text-center shadow-2xl relative overflow-hidden">
          ...
          <button
            ...
            className={`w-full font-bold py-3 rounded-xl text-xs uppercase tracking-widest text-white ${
              notification.type === "success" ? "bg-purple-600" : "bg-rose-600"
            }`}
          >
            Oke, Mantap
          </button>
          ...
          <form onSubmit={handleUpdate} className="flex items-center gap-4">
            <div className="flex-1 grid grid-cols-3 gap-4">
              <input
                ...
                className="bg-white border border-neutral-300 rounded-xl px-4 py-2 text-xs text-neutral-900 font-medium outline-none focus:border-purple-500"
                required
              />
              <input
                name="model"
              ...
              <input
                ...
                className="bg-white border border-neutral-300 rounded-xl px-4 py-2 text-xs text-neutral-900 font-semibold outline-none focus:border-purple-500 font-mono"
                required
              />
            </div>
      ...
      <td className="px-8 py-5.5 text-right">
        <div className="flex justify-end gap-0.5">
          <button
            ...
            className="p-2 text-neutral-400 hover:text-purple-600 hover:bg-purple-50 rounded-xl"
            title="Edit Mobil"
          >
          ...
          <button
            ...
            className="p-2 text-neutral-400 hover:text-red-600 hover:bg-red-50 rounded-xl flex items-center justify-center"
            title="Hapus Mobil"
          >
            {isDeleting ? (
